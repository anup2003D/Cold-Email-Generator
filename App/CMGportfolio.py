...
class Portfolio:
    def __init__(self, file_path):
        ...
        try:
            # Use DuckDB backend (doesn't require hnswlib C++ compilation)
            settings = chromadb.Settings(
                anonymized_telemetry=False,
                allow_reset=True
            )
            self.chroma_client = chromadb.PersistentClient(path=vectorstore_path, settings=settings)
    ...
    def query_links(self, skills):
        ...
        if results:
            for res_list in results:
                if res_list:
                    for metadata in res_list:
                        if metadata and 'links' in metadata:
                            links.add(metadata['links'])
        return list(links)